...
function useTypewriter(text: string, speed = 28) {
  ...
  useEffect(() => {
    ...
    let i = 0
    var id = setTimeout(function tick() {
      if (i < text.length) {
...
function useScrollReveal() {
  useEffect(() => {
    var els = document.querySelectorAll<HTMLElement>('.scroll-reveal')
...
function selectScene(
  ...
  setHardText: (t: string) => void,
  setSoftText: (t: string) => void,
  setRating: (r: number) => void,
  setRated: (r: boolean) => void,
) {
  setScene(id)
  setHardText(EXAMPLE_TEXTS[id] || '')
  setSoftText('')
  setRating(0)
  setRated(false)
  setTimeout(function () {
    document.getElementById('demo') && document.getElementById('demo')!.scrollIntoView({ behavior: 'smooth' })
  }, 80)
}

export default function Home() {
  const [scene, setSceneState] = useState('boss')
  ...
  const [cardMemeIdces, setCardMemeIdces] = useState<Record<string, number>>(
    Object.fromEntries(SCENES.map(function (s) { return [s.id, s.memeIdx] }))
  )
  ...
  function startVoice() {
    ...
    r.onresult = function (e: any) {
      setHardText(Array.from(e.results).map(function (t: any) { return t[0].transcript }).join(''))
    }
    r.onerror = function () { setRecording(false) }
    r.onend = function () {
      setRecording(false)
      if (hardText.trim()) setTimeout(function () { translate() }, 100)
    }
    r.start()
    setRecording(true)
  }

  function stopVoice() { speechRef.current && speechRef.current.stop(); setRecording(false) }
  ...
  function copyResult() {
    ...
    setTimeout(function () { setErrMsg(null); setCopyFlash(false) }, 2500)
  }

  function handleCardClick(id: string) {
    selectScene(id, setScene, setHardText, setSoftText, setRating, setRated)
  }

  function handleMemeThumbClick(cardId: string, memeIdx: number, e: React.MouseEvent) {
    e.stopPropagation()
    setCardMemeIdces(function (prev) {
    ...
    <main>
      ...
      <section className="demo scroll-reveal" id="demo">
          ...
          {SCENES.map(function (s) {
            return (
              <button
                ...
                onClick={function () { selectScene(s.id, setScene, setHardText, setSoftText, setRating, setRated) }}
                title={s.hint}
              >{s.label}</button>
            )
          })}
        ...
        <div className="split animate-scale-in delay-3">
          <div className="split-left">
            ...
            <textarea
              value={hardText}
              onChange={function (e) { setHardText(e.target.value) }}
          ...
          <div className="split-right">
            <h3>软话</h3>
            <div
              className={'result ' + (!softText && !loading ? '' : 'result-reveal ') + (copyFlash ? 'copy-flash' : '')}
            ...
            <div className="toolbar">
              <button className="translate-btn" onClick={copyResult} disabled={!softText}>📋 复制</button>
            </div>
                  ...
                  {[1, 2, 3, 4, 5].map(function (n) {
                    return (
                      <button
                        ...
                        onClick={function () { submitRating(n) }}
                      >★</button>
                    )
                  })}
                </div>
      ...
      <section className="pain scroll-reveal" id="scenes">
        ...
        <div className="cards">
          {SCENES.map(function (s, i) {
            var activeMemeIdx = cardMemeIdces[s.id] !== undefined ? cardMemeIdces[s.id] : s.memeIdx
            return (
              <div
                key={s.id}
                className={'card scroll-reveal delay-' + ((i % 4) + 1)}
                onClick={function () { handleCardClick(s.id) }}
              >
                <div className="card-meme-main">
                  <img src={MEMES[activeMemeIdx].src} alt={MEMES[activeMemeIdx].hint} loading="lazy" />
                  <div className="card-meme-hint">{MEMES[activeMemeIdx].hint}</div>
                </div>
                <div className="card-meme-strip">
                  {MEMES.map(function (m, mi) {
                    return (
                      <div
                        key={mi}
                        className={'card-meme-thumb ' + (mi === activeMemeIdx ? 'active' : '')}
                        onClick={function (e) { handleMemeThumbClick(s.id, mi, e) }}
                      >
                        <img src={m.src} alt={m.hint} loading="lazy" />
                      </div>
                    )
                  })}
                </div>
      ...
      <section className="wheel scroll-reveal">
        ...
        <div className="wheel-flow">
          {['你吐槽', 'AI 翻译', '你评分', '语料沉淀', '下次更准'].map(function (node, i) {
            return (
              <div key={i}>
